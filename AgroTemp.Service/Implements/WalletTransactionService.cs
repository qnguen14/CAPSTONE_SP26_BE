        ...
        public async Task ApplyJobSettlementAsync(JobDetail jobDetail, decimal workerPaymentAmount, decimal refundAmount)
        {
            ...
            try
            {
                if (workerPaymentAmount > 0)
                {
                    ...
                    _unitOfWork.GetRepository<Wallet>().UpdateAsync(workerWallet);
                }
                ...
                if (refundAmount > 0)
                {
                    ...
                    _unitOfWork.GetRepository<Wallet>().UpdateAsync(farmerWallet);
                }
            }
            catch (Exception ex) when (IsSchemaMismatch(ex))
            {
                // Fallback for legacy schema (Wallets/WalletTransactions with PascalCase columns).
                if (workerPaymentAmount > 0)
                {
