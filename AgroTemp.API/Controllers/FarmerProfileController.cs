...
public class FarmerProfileController : Controller
{
    ...
    [HttpGet(ApiEndpointConstants.FarmerProfile.GetSpecificFarmerProfileEndpoint)]
    [ProducesResponseType(typeof(ApiResponse<FarmerProfileDTO>), StatusCodes.Status200OK)]
    [ProducesResponseType(typeof(ApiResponse<object>), StatusCodes.Status403Forbidden)]
    [ProducesResponseType(typeof(ApiResponse<object>), StatusCodes.Status404NotFound)]
    [ProducesResponseType(typeof(ApiResponse<object>), StatusCodes.Status500InternalServerError)]
    public async Task<ActionResult<FarmerProfileDTO>> GetFarmerProfileById([FromRoute] Guid id)
    {
        try
        {
            var profile = await _userService.GetFarmerProfileById(id);
