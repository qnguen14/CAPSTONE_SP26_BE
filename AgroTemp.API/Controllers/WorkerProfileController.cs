...
public class WorkerProfileController : Controller
{
    ...
    /// Get worker profile by user ID - Only accessible by the worker who owns the profile
    /// </summary>
    [HttpGet(ApiEndpointConstants.WorkerProfile.GetSpecificWorkerProfileEndpoint)]
    [ProducesResponseType(typeof(ApiResponse<WorkerProfileDTO>), StatusCodes.Status200OK)]
    [ProducesResponseType(typeof(ApiResponse<object>), StatusCodes.Status403Forbidden)]
    [ProducesResponseType(typeof(ApiResponse<object>), StatusCodes.Status404NotFound)]
    [ProducesResponseType(typeof(ApiResponse<object>), StatusCodes.Status500InternalServerError)]
    public async Task<ActionResult<WorkerProfileDTO>> GetWorkerProfileById([FromRoute] Guid id)
    {
        try
        {
            var profile = await _userService.GetWorkerProfileById(id);
